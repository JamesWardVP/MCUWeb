...
- [`tools/refresh-media-data.ps1`](../tools/refresh-media-data.ps1) reads the timeline entries from `data/timeline.json`, matches each to TMDB (and OMDb, for the IMDb number), and writes the results to `data/media-cache.json`, keyed by each entry's stable `id`.
...
- The site (`app.js`) fetches `data/media-cache.json` once on load and looks up each entry by its stable node `id` (see [item 4](04-timeline-editor-backend.md)) — it never calls TMDB or OMDb directly, and neither API key ever ships to the browser (they're GitHub Actions secrets, used only inside the workflow).
...
Anything the automatic matching gets wrong (or can't find) can be corrected by hand in [`data/media-overrides.json`](../data/media-overrides.json): `{ "<node-id>": { "tmdbId": 123, "mediaType": "movie" } }`.
