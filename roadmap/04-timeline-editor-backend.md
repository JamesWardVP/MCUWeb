...
- **Add and Edit are now separate, distinctly-styled UIs**: "Add Entry" is its own tab (green accent), "Manage Entries" holds search/list/delete plus an amber-accented "Editing: …" panel that only appears once you click Edit — no more one form silently relabelling itself between two modes.
- **Visual flowchart-position picker**: both the Add and Edit forms show a small pannable/zoomable map of the current flowchart (all existing entries rendered at their real position/colour) instead of asking for bare X/Y numbers. Click anywhere to drop the entry there — the click centers it under the cursor and shows a dashed outline + pin — or fine-tune with the number fields afterward, which stay in sync with the pin either way.
